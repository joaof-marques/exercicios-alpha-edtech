...
valorTotalEstoque = 0
for i in range(len(produtos_disponiveis)):
    produtos_disponiveis[i] = tuple([produtos_disponiveis[i][0], produtos_disponiveis[i][1], produtos_disponiveis[i][2], produtos_disponiveis[i][3], round(produtos_disponiveis[i][2]*produtos_disponiveis[i][3], 2)])
    valorTotalEstoque += produtos_disponiveis[i][4]
        
print("\nEstoque Final:\n")
for i in produtos_disponiveis:
    print(f"Nome: {i[0]}\t|\tCódigo: {i[1]}\t|\tQuantidade em estoque: {i[2]}\t|\tPreço: R$ {i[3]:.2f}\t|\tValor Total Produto: {i[4]}")
print(f"------ Valor total do estoque: {valorTotalEstoque} ------")
